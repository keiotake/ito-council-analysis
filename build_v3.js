...
<meta name="description" content="伊東市議会の議員活動報告・議会情報のまとめサイト。議員プロフィール・議会動画・質問要約・制度案内を掲載。">
...
.hero-meta-label{font-family:var(--font-en);font-size:clamp(.46rem,1.2vw,.56rem);font-weight:500;letter-spacing:.24em;color:rgba(255,255,255,.55);text-transform:uppercase;border-right:1px solid rgba(255,255,255,.2);padding-right:clamp(.4rem,1.3vw,.75rem);flex-shrink:0;white-space:nowrap}
.hero-meta-name{font-size:clamp(.66rem,1.7vw,.74rem);font-weight:400;color:rgba(255,255,255,.75);letter-spacing:.02em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.hero-meta-name strong{font-weight:600;color:rgba(255,255,255,.92);letter-spacing:.04em;margin-left:.25rem}
...
/* ビジョンセクション（ヒーロー直下） */
.vision-section{background:linear-gradient(180deg,#fafafa 0%,#fff 100%);padding:clamp(3rem,7vw,5rem) clamp(1rem,4vw,2rem);border-bottom:1px solid var(--border)}
.vision-inner{max-width:1100px;margin:0 auto}
.vision-badge{display:inline-block;font-family:var(--font-en);font-size:.7rem;font-weight:700;letter-spacing:.36em;color:#2563eb;background:rgba(37,99,235,.08);border:1px solid rgba(37,99,235,.2);border-radius:999px;padding:.4rem 1rem;margin-bottom:1.2rem}
...
.vision-card .vc-desc{font-size:clamp(.78rem,1.6vw,.85rem);color:#475569;line-height:1.75;margin:0}
.hide-on-mobile{display:initial}
@media(max-width:900px){
  .vision-cards{grid-template-columns:1fr;gap:.9rem}
  .vision-card{padding:1.3rem 1.4rem}
  .vision-card .vc-num{font-size:1.8rem}
}
@media(max-width:640px){
  .vision-section{padding:2.5rem 1rem 3rem}
  .vision-title{font-size:1.15rem;line-height:1.55}
...
/* タブごとの色分け（カラーコーディング） */
/* 議員一覧 = 青系（信頼・人物） */
.nav-tab.nav-blue{color:#1e40af}
.nav-tab.nav-blue:hover{background:#dbeafe;border-color:#93c5fd;color:#1e3a8a}
.nav-tab.nav-blue.active{background:#2563eb;color:#fff;border-color:#2563eb;box-shadow:0 4px 12px rgba(37,99,235,.3)}
/* テーマから探す = 紫系（探索・発見） */
.nav-tab.nav-purple{color:#6d28d9}
.nav-tab.nav-purple:hover{background:#ede9fe;border-color:#c4b5fd;color:#5b21b6}
.nav-tab.nav-purple.active{background:#7c3aed;color:#fff;border-color:#7c3aed;box-shadow:0 4px 12px rgba(124,58,237,.3)}
/* 市民の声 = 緑系（コミュニティ） */
.nav-tab.nav-green{color:#047857}
.nav-tab.nav-green:hover{background:#d1fae5;border-color:#6ee7b7;color:#065f46}
.nav-tab.nav-green.active{background:#10b981;color:#fff;border-color:#10b981;box-shadow:0 4px 12px rgba(16,185,129,.3)}
/* 市政に参加する = オレンジ系（行動） */
...
/* 動画・全文検索 = ピンク系 */
.nav-tab.nav-rose{color:#be185d}
.nav-tab.nav-rose:hover{background:#fce7f3;border-color:#f9a8d4;color:#9d174d}
.nav-tab.nav-rose.active{background:#e11d48;color:#fff;border-color:#e11d48;box-shadow:0 4px 12px rgba(225,29,72,.3)}
/* 総合計画 = ティール系 */
.nav-tab.nav-teal{color:#0f766e}
.nav-tab.nav-teal:hover{background:#ccfbf1;border-color:#5eead4;color:#115e59}
.nav-tab.nav-teal.active{background:#14b8a6;color:#fff;border-color:#14b8a6;box-shadow:0 4px 12px rgba(20,184,166,.3)}
/* 伊東市分析 = アンバー系 */
.nav-tab.nav-amber{color:#a16207}
.nav-tab.nav-amber:hover{background:#fef3c7;border-color:#fcd34d;color:#854d0e}
.nav-tab.nav-amber.active{background:#d97706;color:#fff;border-color:#d97706;box-shadow:0 4px 12px rgba(217,119,6,.3)}
/* 議会全体 = グレー系 */
.nav-tab.nav-gray{color:#52525b}
.nav-tab.nav-gray:hover{background:#e4e4e7;border-color:#d4d4d8;color:#27272a}
.nav-tab.nav-gray.active{background:#3f3f46;color:#fff;border-color:#3f3f46;box-shadow:0 4px 12px rgba(63,63,70,.3)}
...
.search-input{flex:1;min-width:200px;padding:.9rem 1.3rem;border:1px solid var(--border);border-radius:14px;font-size:1rem;outline:none;transition:.2s;background:#fff;box-shadow:var(--shadow-sm)}
...
.m-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:1.2rem;margin-bottom:1.5rem}
.m-card{background:var(--card);border-radius:18px;padding:1.4rem 1rem;text-align:center;cursor:pointer;transition:transform .25s,box-shadow .25s,border-color .25s;box-shadow:var(--shadow-sm);border:1px solid var(--border);overflow:hidden;position:relative}
.m-card:hover{transform:translateY(-3px);box-shadow:var(--shadow-md);border-color:#0f172a}
.m-card.is-rep{border-color:#fbbf24}
.m-card.is-rep:hover{border-color:#f59e0b;box-shadow:0 8px 24px rgba(245,158,11,.18)}
.rep-badge{position:absolute;top:.7rem;left:.7rem;background:#fbbf24;color:#78350f;font-size:.65rem;font-weight:700;padding:.2rem .55rem;border-radius:999px;letter-spacing:.04em;z-index:2}
.m-avatar{width:88px;height:88px;border-radius:50%;margin:0 auto .8rem;display:flex;align-items:center;justify-content:center;font-size:2.4rem;font-weight:800;color:#fff;box-shadow:0 4px 12px rgba(0,0,0,.1);background-image:linear-gradient(135deg,rgba(255,255,255,.2),rgba(0,0,0,.08))}
.m-avatar-photo{width:88px;height:88px;border-radius:50%;margin:0 auto .8rem;overflow:hidden;border:3px solid;box-shadow:0 4px 12px rgba(0,0,0,.08)}
.m-avatar-photo img{width:100%;height:100%;object-fit:cover}
.m-avatar-fb{width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:2.4rem;font-weight:800;color:#fff;background-image:linear-gradient(135deg,rgba(255,255,255,.2),rgba(0,0,0,.08))}
.m-name{font-size:1.1rem;font-weight:800;margin-bottom:.3rem;letter-spacing:-.005em}
.m-faction{font-size:.78rem;font-weight:700;margin-bottom:.4rem;letter-spacing:.02em}
.m-committee{font-size:.74rem;color:var(--sub);margin-bottom:.4rem}
...
.detail-avatar{width:100px;height:100px;border-radius:50%;margin:0 auto .8rem;display:flex;align-items:center;justify-content:center;font-size:2.8rem;font-weight:800;color:#fff;border:4px solid rgba(255,255,255,.7);box-shadow:0 3px 10px rgba(0,0,0,.15);background-image:linear-gradient(135deg,rgba(255,255,255,.18),rgba(0,0,0,.05))}
...
footer{padding:2.5rem 1.5rem 1.5rem;color:var(--sub);font-size:.82rem;background:#0f172a;color:#cbd5e1;margin-top:3rem}
.footer-grid{max-width:1100px;margin:0 auto;display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1.5rem;padding-bottom:1.5rem;border-bottom:1px solid rgba(255,255,255,.1)}
.footer-block{}
.footer-title{font-size:.85rem;font-weight:700;color:#fff;margin-bottom:.6rem;letter-spacing:.02em}
.footer-text{font-size:.78rem;line-height:1.7;color:#94a3b8}
.footer-text small{font-size:.7rem;opacity:.85}
.footer-link{color:#93c5fd;text-decoration:none;font-weight:600}
.footer-link:hover{color:#fff;text-decoration:underline}
.footer-credit-line{max-width:1100px;margin:1.2rem auto 0;text-align:center;font-size:.72rem;color:#64748b;padding-top:.8rem}
@media(max-width:640px){footer{padding:2rem 1rem 1.2rem}.footer-grid{gap:1.2rem}.footer-credit-line{font-size:.65rem;line-height:1.6}}
...
/* 議員の方へタブ */
.mp-hero{background:linear-gradient(135deg,#1e293b 0%,#0f172a 50%,#1e3a8a 100%);color:#fff;border-radius:24px;padding:3rem 2rem;margin-bottom:2rem;text-align:center;position:relative;overflow:hidden}
.mp-hero::before{content:'';position:absolute;top:-30%;right:-15%;width:500px;height:500px;background:radial-gradient(circle,rgba(99,102,241,.3),transparent 70%);pointer-events:none}
.mp-hero-inner{position:relative;z-index:1;max-width:720px;margin:0 auto}
.mp-hero-badge{display:inline-block;background:rgba(255,255,255,.18);padding:.35rem .9rem;border-radius:999px;font-size:.78rem;font-weight:700;letter-spacing:.04em;margin-bottom:1rem}
.mp-hero h1{font-size:1.8rem;font-weight:800;letter-spacing:-.01em;margin-bottom:1rem}
.mp-hero p{font-size:.95rem;opacity:.95;line-height:1.8;margin-bottom:1.5rem}
.mp-cta-btn{display:inline-block;padding:.95rem 1.8rem;background:#fff;color:#0f172a;border-radius:999px;font-size:.95rem;font-weight:800;text-decoration:none;box-shadow:0 6px 20px rgba(0,0,0,.25);transition:.2s}
.mp-cta-btn:hover{transform:translateY(-2px);box-shadow:0 10px 28px rgba(0,0,0,.35)}

.mp-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(310px,1fr));gap:1.2rem;margin-bottom:2rem}
.mp-card{background:#fff;border:1px solid var(--border);border-radius:18px;padding:1.6rem 1.5rem;box-shadow:var(--shadow-sm);transition:all .25s}
.mp-card:hover{transform:translateY(-3px);box-shadow:var(--shadow-md)}
.mp-card-add{border-top:4px solid #2563eb}
.mp-card-photo{border-top:4px solid #7c3aed}
.mp-card-delete{border-top:4px solid #dc2626}
.mp-card-other{border-top:4px solid #10b981}
.mp-icon{font-size:2rem;margin-bottom:.5rem}
.mp-card h2{font-size:1.2rem;font-weight:800;color:#0a0a0a;margin-bottom:.6rem;letter-spacing:-.01em}
.mp-card>p{font-size:.88rem;color:#475569;line-height:1.7;margin-bottom:.9rem}
.mp-card>p strong{color:#dc2626;background:#fef2f2;padding:.05rem .3rem;border-radius:4px}
.mp-list{padding-left:1.3rem;margin:.6rem 0 1.2rem}
.mp-list li{font-size:.83rem;color:#374151;line-height:1.7;margin-bottom:.25rem}
.mp-action-btn{display:block;text-align:center;padding:.75rem 1rem;border-radius:10px;font-size:.85rem;font-weight:700;text-decoration:none;transition:.15s}
.mp-action-btn:hover{transform:translateY(-1px);box-shadow:var(--shadow-md)}
.mp-btn-blue{background:#2563eb;color:#fff}
.mp-btn-blue:hover{background:#1d4ed8}
.mp-btn-purple{background:#7c3aed;color:#fff}
.mp-btn-purple:hover{background:#6d28d9}
.mp-btn-red{background:#dc2626;color:#fff}
.mp-btn-red:hover{background:#b91c1c}
.mp-btn-green{background:#10b981;color:#fff}
.mp-btn-green:hover{background:#059669}

.mp-policy{background:#f8fafc;border:1px solid var(--border);border-radius:14px;padding:1.4rem 1.6rem;margin-bottom:1rem}
.mp-policy h3{font-size:1rem;font-weight:800;margin-bottom:.8rem;color:#0a0a0a}
.mp-policy ul{padding-left:1.3rem;margin:0}
.mp-policy li{font-size:.82rem;color:#374151;line-height:1.85;margin-bottom:.3rem}
.mp-policy strong{color:#0f172a;font-weight:700}
.mp-policy a{color:#2563eb;text-decoration:underline;font-weight:600}
@media(max-width:640px){.mp-hero{padding:2rem 1.2rem}.mp-hero h1{font-size:1.4rem}.mp-cta-btn{font-size:.85rem;padding:.75rem 1.3rem}.mp-card{padding:1.3rem 1.1rem}}
...
.theme-btn{padding:.55rem 1rem;border:1px solid var(--border);border-radius:999px;background:#fff;font-size:.83rem;font-weight:600;cursor:pointer;transition:all .2s;color:var(--text);box-shadow:var(--shadow-sm)}
...
/* 議員詳細のコンパクトヘッダー */
.detail-compact-header{display:flex;gap:1.2rem;align-items:center;padding:1.5rem 1.8rem;background:#fff;border-radius:20px;margin-bottom:1rem;border:1px solid var(--border);box-shadow:var(--shadow-sm)}

/* 活動サマリーパネル */
.detail-summary{background:#fff;border:1px solid var(--border);border-radius:20px;padding:1.4rem 1.6rem;margin-bottom:1rem;box-shadow:var(--shadow-sm)}
...
.dch-name{font-size:1.7rem;font-weight:900;color:#0a0a0a;margin:0 0 .35rem 0;letter-spacing:-.02em;line-height:1.2}
.dch-meta{display:flex;flex-wrap:wrap;gap:.5rem;align-items:center;font-size:.85rem}
...
/* 用語解説フローティングボタン */
.explain-btn{position:absolute;z-index:9998;padding:.5rem 1rem;background:linear-gradient(135deg,#8b5cf6,#7c3aed);color:#fff;border:none;border-radius:20px;font-size:.82rem;font-weight:700;cursor:pointer;box-shadow:0 4px 14px rgba(124,58,237,.4);animation:explainPop .2s ease-out;white-space:nowrap}
.explain-btn:hover{transform:translateY(-1px);box-shadow:0 6px 18px rgba(124,58,237,.5)}
@keyframes explainPop{from{transform:scale(.85);opacity:0}to{transform:scale(1);opacity:1}}

/* 用語解説モーダル */
.explain-overlay{display:none;position:fixed;inset:0;background:rgba(15,23,42,.7);z-index:3000;align-items:center;justify-content:center;padding:1.5rem;backdrop-filter:blur(4px)}
.explain-overlay.open{display:flex;animation:fadeIn .2s}
.explain-box{background:#fff;border-radius:20px;max-width:520px;width:100%;box-shadow:0 25px 60px rgba(0,0,0,.4);overflow:hidden;animation:slideUp .25s;position:relative}
.explain-close{position:absolute;top:.7rem;right:.7rem;background:rgba(0,0,0,.08);border:none;border-radius:50%;width:32px;height:32px;font-size:.95rem;cursor:pointer;color:#64748b;display:flex;align-items:center;justify-content:center;z-index:1}
.explain-close:hover{background:rgba(0,0,0,.15);color:#1e293b}
.explain-header{background:linear-gradient(135deg,#8b5cf6,#6d28d9);color:#fff;padding:1.3rem 1.5rem;display:flex;align-items:center;gap:.8rem}
.explain-icon{font-size:2rem}
.explain-label{font-size:.75rem;opacity:.85;letter-spacing:.05em;margin-bottom:.2rem}
.explain-term{font-size:1.25rem;font-weight:800;word-break:break-all}
.explain-body{padding:1.5rem;max-height:60vh;overflow-y:auto;min-height:180px}
.explain-loading{display:flex;flex-direction:column;align-items:center;gap:1rem;padding:2rem 0;color:#64748b;font-size:.88rem}
.spinner{width:32px;height:32px;border:3px solid #e9d5ff;border-top-color:#7c3aed;border-radius:50%;animation:spin .8s linear infinite}
@keyframes spin{to{transform:rotate(360deg)}}
.explain-summary{font-size:1.05rem;font-weight:700;color:#1e293b;line-height:1.7;margin-bottom:1rem;padding:1rem;background:linear-gradient(135deg,#faf5ff,#f3e8ff);border-radius:10px;border-left:4px solid #8b5cf6}
.explain-detail{font-size:.9rem;line-height:1.8;color:#334155;margin-bottom:1rem;padding:0 .3rem}
.explain-detail-label,.explain-example-label{font-size:.75rem;font-weight:700;color:#7c3aed;margin-bottom:.3rem;letter-spacing:.03em}
...
/* 市民参加ガイドタブ */
.participate-hero{background:linear-gradient(135deg,#0a0a0a 0%,#1e293b 100%);color:#fff;border-radius:24px;padding:3rem 2rem;margin-bottom:2rem;text-align:center;position:relative;overflow:hidden}
.participate-hero::before{content:'';position:absolute;top:-50%;right:-20%;width:600px;height:600px;background:radial-gradient(circle,rgba(16,185,129,.25),transparent 70%);pointer-events:none}
...
.participate-hero p{font-size:.95rem;opacity:.95;line-height:1.75;max-width:720px;margin:0 auto 1.2rem}
.participate-hero h1{position:relative;z-index:1}
.participate-hero p{position:relative;z-index:1}
.participate-quicklinks{display:flex;gap:.6rem;justify-content:center;flex-wrap:wrap;position:relative;z-index:1;margin-top:1.2rem}
...
/* テーマ横断ビュータブ */
.themes-hero{background:linear-gradient(135deg,#0a0a0a 0%,#1e3a8a 100%);color:#fff;border-radius:24px;padding:2.5rem 2rem;margin-bottom:2rem;text-align:center;position:relative;overflow:hidden}
.themes-hero::before{content:'';position:absolute;top:-50%;left:-15%;width:500px;height:500px;background:radial-gradient(circle,rgba(139,92,246,.25),transparent 70%);pointer-events:none}
...
<section class="vision-section">
  <div class="vision-inner">
    ...
    <h2 class="vision-title">市民の声を集め、伊東市の現状を可視化し、<br class="hide-on-mobile">これからをみんなで考える「場」へ。</h2>
    <div class="vision-cards">
      <div class="vision-card">
        <div class="vc-num">01</div>
        <div class="vc-label">COLLECT</div>
        <div class="vc-title">市民の声を集める</div>
        <p class="vc-desc">街の良いところ・改善してほしいこと・アイデアを、市民の皆さまから自由に投稿いただけます。集まった声は議会や行政に届ける材料として活用します。</p>
      </div>
      <div class="vision-card">
        <div class="vc-num">02</div>
        <div class="vc-label">ANALYZE</div>
        <div class="vc-title">現状を定量的に可視化する</div>
        <p class="vc-desc">人口推計・財政・観光・産業など、公的データに基づく客観的な「街の今」をわかりやすく見える化します。議会で何が議論されているかも整理します。</p>
      </div>
      <div class="vision-card">
        <div class="vc-num">03</div>
        <div class="vc-label">DISCUSS</div>
        <div class="vc-title">打開策をみんなで考える</div>
        <p class="vc-desc">市民の声と定量データを突き合わせ、「次に伊東市はどう動くべきか」を市民・議員・行政が一緒に考える土台にしたい——そんな思いで運営しています。</p>
      </div>
    </div>
  </div>
</section>
...
<nav role="tablist" aria-label="メインナビゲーション">
  ...
  <button class="nav-tab nav-rose" role="tab" aria-selected="false" onclick="switchTab('all',this)">🎥 動画・全文検索</button>
  <button class="nav-tab nav-teal" role="tab" aria-selected="false" onclick="switchTab('plan',this)">📘 総合計画</button>
  <button class="nav-tab nav-amber" role="tab" aria-selected="false" onclick="switchTab('analysis',this)">🔎 伊東市分析</button>
  <button class="nav-tab nav-gray" role="tab" aria-selected="false" onclick="switchTab('stats',this)">📊 議会全体</button>
</nav>
<div class="container" id="main-content" role="main">
  ...
  <div id="tab-participate" class="tab-panel">
    <div class="participate-hero">
      ...
      <div class="participate-quicklinks">
        ...
        <a href="#pg-chinjo" class="pql-btn">📜 陳情・請願</a>
        <a href="#pg-vote" class="pql-btn">🗳️ 選挙情報</a>
      </div>
    </div>

    <div class="participate-grid">
      <div class="pg-card" id="pg-pubcom">
      ...
      <div class="pg-card" id="pg-chinjo">
        <div class="pg-icon">📝</div>
        <h2>陳情・請願</h2>
  ...
  <!-- 議員の方へタブ（議員専用窓口） -->
  <div id="tab-memberportal" class="tab-panel">
    <div class="mp-hero">
    ...
    <div class="mp-grid">
      <div class="mp-card mp-card-add">
        <div class="mp-icon">➕</div>
        <h2>情報の追加・修正</h2>
        <p>プロフィール、所属委員会、政策、経歴などの追記・訂正をご依頼ください。</p>
        <ul class="mp-list">
          <li>正確なプロフィール情報の提供</li>
          <li>議員からの一言コメント追加</li>
          <li>政策・公約の掲載</li>
          <li>経歴・実績の追記</li>
        </ul>
        <a href="mailto:ka@oh-life.co.jp?subject=【みんなの伊東市】情報の追加・修正依頼&body=議員名：%0D%0A%0D%0A追加・修正したい項目：%0D%0A%0D%0A具体的な内容：%0D%0A" class="mp-action-btn mp-btn-blue">📝 追加・修正を依頼する</a>
      </div>

      <div class="mp-card mp-card-photo">
        <div class="mp-icon">📷</div>
        <h2>顔写真の提供・差し替え</h2>
        <p>ご自身でご用意された顔写真への差し替え・追加が可能です。提供いただいた写真は本サイトでのみ使用いたします。</p>
        <ul class="mp-list">
          <li>正面・上半身の写真を推奨</li>
          <li>JPG / PNG 形式</li>
          <li>500×500px 以上を推奨</li>
          <li>他者の著作権を侵害しないもの</li>
        </ul>
        <a href="mailto:ka@oh-life.co.jp?subject=【みんなの伊東市】顔写真提供&body=議員名：%0D%0A%0D%0A※顔写真ファイルをこのメールに添付してお送りください。%0D%0A%0D%0A使用許諾：本サイト「みんなの伊東市」での掲載に同意します。%0D%0A" class="mp-action-btn mp-btn-purple">📎 写真をメールで送る</a>
      </div>
      ...
      <div class="mp-card mp-card-delete">
        ...
        <a href="mailto:ka@oh-life.co.jp?subject=【みんなの伊東市】削除依頼（緊急）&body=議員名：%0D%0A%0D%0A削除を希望する情報：%0D%0A%0D%0A理由（任意）：%0D%0A" class="mp-action-btn mp-btn-red">🚨 削除を依頼する</a>
      </div>

      <div class="mp-card mp-card-other">
        <div class="mp-icon">💬</div>
        <h2>その他のご相談</h2>
        <p>サイトの内容・運営に関するご意見、改善のご提案など、何でもお気軽にお寄せください。</p>
        <ul class="mp-list">
          <li>サイトの仕様についての質問</li>
          <li>新機能・改善のご提案</li>
          <li>連携・相互リンクのご相談</li>
        ...
        <a href="mailto:ka@oh-life.co.jp?subject=【みんなの伊東市】お問い合わせ&body=お名前：%0D%0A%0D%0Aお問い合わせ内容：%0D%0A" class="mp-action-btn mp-btn-green">💌 メールで相談する</a>
      </div>
    </div>
...
<footer>
  <div class="footer-grid">
    <div class="footer-block">
      <div class="footer-title">📋 運営情報</div>
      <div class="footer-text">
        運営責任者：伊東市議会議員 大竹 圭<br>
        非公式の市民向け情報サイト
      </div>
    </div>
    <div class="footer-block">
    ...
    <div class="footer-block">
      <div class="footer-title">📚 データ出典</div>
      <div class="footer-text">
        <a href="https://www.youtube.com/channel/UC9FGDfo93b_dpu_7-AnN4wQ" target="_blank" class="footer-link">伊東市議会YouTube</a><br>
        <a href="https://www.city.ito.shizuoka.jp/gyosei/shiseijoho/itoshigikai/index.html" target="_blank" class="footer-link">伊東市議会公式HP</a>
      </div>
    </div>
    <div class="footer-block">
      <div class="footer-title">⚠️ ご注意</div>
      <div class="footer-text">
        <a href="javascript:void(0)" onclick="openLimitsModal()" class="footer-link">📖 このデータの限界について</a><br>
        <small>正確な内容は公式記録でご確認ください</small>
      </div>
    </div>
  </div>
...
<!-- 用語解説モーダル -->
<div class="explain-overlay" id="explain-overlay" onclick="if(event.target===this)closeExplainModal()">
  <div class="explain-box">
    <button class="explain-close" onclick="closeExplainModal()">✕</button>
    ...
    <div class="explain-body" id="explain-body">
      <div class="explain-loading">
        ...
        <div>AIが解説を考えています…</div>
      </div>
    </div>
    <div class="explain-footer">
      <span class="explain-powered">🤖 Claude AI による自動解説</span>
    </div>
  </div>
</div>
...
// === 用語解説機能 ===
let currentExplainTerm = '';
let currentExplainContext = '';
...
(function initExplain(){
  ...
  document.addEventListener('click', (e) => {
    if (e.target && !e.target.closest('#explain-btn') && !e.target.closest('.explain-box')) {
      setTimeout(() => {
        if (window.getSelection().toString().trim().length === 0) hideExplainBtn();
      }, 10);
    }
  });
  ...
  function handleSelection(e){
    ...
    if (!sel) { hideExplainBtn(); return; }
    const text = sel.toString().trim();
    // 1〜30文字の選択のみ解説対象
    if (text.length < 1 || text.length > 30) { hideExplainBtn(); return; }
    // モーダル内の選択は除外
    if (e && e.target && e.target.closest('.explain-box')) return;
    // ナビ、ヘッダー、フッターからの選択は除外
    if (e && e.target && (e.target.closest('header') || e.target.closest('nav') || e.target.closest('.back-btn'))) { hideExplainBtn(); return; }
    currentExplainTerm = text;
    // 周辺のテキストをコンテキストとして取得
    let context = '';
    try {
...
function openExplainModal(){
  ...
  document.getElementById('explain-term').textContent = '「' + currentExplainTerm + '」';
  document.getElementById('explain-body').innerHTML = '<div class="explain-loading"><div class="spinner"></div><div>AIが解説を考えています…</div></div>';
  overlay.classList.add('open');
  document.body.style.overflow = 'hidden';
  // API呼び出し
  fetch(VOICE_API + '/explain', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ term: currentExplainTerm, context: currentExplainContext }),
  })
  .then(r => r.json())
  .then(data => {
    ...
    let html = '';
    if (data.summary) {
      html += '<div class="explain-summary">' + escapeHtml(data.summary) + '</div>';
    }
    if (data.explain) {
      html += '<div class="explain-detail"><div class="explain-detail-label">📖 詳しく</div>' + escapeHtml(data.explain) + '</div>';
    }
    if (data.example) {
  ...
  .catch(err => {
    document.getElementById('explain-body').innerHTML = '<div class="explain-error">⚠️ 通信エラー: ' + escapeHtml(err.message) + '</div>';
  });
}
